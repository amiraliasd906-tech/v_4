...
object OwnerDetector {
    ...
    fun heuristicAgencyProbability(description: String?): Double {
        if (description.isNullOrBlank()) return 0.5

        var score = 0.5
        for ((phrase, weight) in AGENCY_KEYWORDS) if (description.contains(phrase)) score += weight
        for ((phrase, weight) in OWNER_KEYWORDS) if (description.contains(phrase)) score -= weight
        if (NEGATION_PATTERN.containsMatchIn(description)) score -= 0.4
     ...
     */
    suspend fun agencyProbability(description: String?, apiKey: String?, model: String): Double {
        ...
        return heuristicAgencyProbability(description)
    }
}
